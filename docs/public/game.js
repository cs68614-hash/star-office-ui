...
async function loadMemo() {
  ...
  try {
    const response = await fetch(__STAR_OFFICE_API_URL__('/yesterday-memo?t=') + Date.now(), { cache: 'no-store' });
...
function setState(state, detail) {
  fetch(__STAR_OFFICE_API_URL__('/set_state'), {
    method: 'POST',
...
function fetchStatus() {
  fetch(__STAR_OFFICE_API_URL__('/status'))
...
function fetchAgents() {
  fetch(__STAR_OFFICE_API_URL__('/agents?t=') + Date.now(), { cache: 'no-store' })
    .then(response => response.json())
    .then(data => {
